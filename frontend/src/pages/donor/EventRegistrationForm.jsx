...
const EventRegistrationForm = () => {
  ...
  const fetchBookedSlots = async () => {
    try {
      const response = await api.get(`/registrations/event/${eventId}/booked-slots`, {
        params: { date: formData.ngay_hien }
      });
      if (response.data.success) {
  ...
  useEffect(() => {
    if (formData.ngay_hien && eventId) {
      fetchBookedSlots();
    } else {
      setBookedSlots([]);
    }
  }, [formData.ngay_hien, eventId]);
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--spacing-lg)' }}>
                ...
                <div className="form-group">
                  ...
                  <select
                    ...
                    className="form-input"
                    style={{
                      color: bookedSlots.includes(formData.khung_gio) ? '#9CA3AF' : 'inherit'
                    }}
                  >
                    <option value="">-- Chọn khung giờ --</option>
                    {khungGio.map((gio, idx) => {
                      const isBooked = bookedSlots.includes(gio);
                      return (
                        <option
                          key={idx}
                          value={gio}
