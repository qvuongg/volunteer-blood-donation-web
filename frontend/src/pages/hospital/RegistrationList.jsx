...
const RegistrationList = () => {
  ...
  const fetchApprovedEvents = async () => {
    ...
    try {
      ...
      toastError('Lỗi khi tải danh sách sự kiện: ' + (error.response?.data?.message || error.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchRegistrations = async (id) => {
    ...
    try {
      ...
      console.error('Error:', error);
      toastError('Lỗi khi tải danh sách đăng ký: ' + (error.response?.data?.message || error.message));
    } finally {
      setLoadingRegs(false);
    }
  };
